...
function _erb_spacing(Fs, Ng, f_low=nothing, f_high=nothing)
    if isnothing(f_high)
        E_high = 21.4 * log10(4.37 * Fs / 2000 + 1)
    elseif f_high <= Fs/2
        E_high = 21.4 * log10(4.37 * f_high /1000 + 1)
    else
        error("f_high must be less than or equal to Fs/2")
    end
    if isnothing(f_low)
        E_low = 0.0
    else
        E_low = 21.4 * log10(4.37 * f_low / 1000 + 1)      
    end
    
    E = range(E_low, E_high, length=Ng)
